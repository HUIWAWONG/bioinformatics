...
# Gene list in --------------------------------
# file_gene_infor="/home/zgu_labs/bin/R/RNAseq/GEP_prediction/Homo_sapiens.GRCh38.V102.summary.txt"
# dux4_genelist=readLines("/home/zgu_labs/bin/R/RNAseq/GEP_prediction/DUX4_gene.list")
...
# gtf_mouse=read_tsv("/ref_genomes/genome_anno/mouse/gtf/V102/Mus_musculus.GRCm38.V102.withChr.summary.txt")
...
correct_batch_var=function(obj_in,batch_var,assay_name_in="vst"){
  ...
  batch_value=unlist(colData(obj_in$SE)[batch_var])
...
get_variable_genes=function(obj_in,N_genes,method="MAD",assay_name_in="vst",feature_panel=NULL){
  if(is.null(feature_panel)){
    df_=assays(obj_in$SE)[[assay_name_in]]
  }
  
  if(!is.null(feature_panel)){
    df_=assays(obj_in$SE)[[assay_name_in]]
    df_=df_[obj_in[[feature_panel]],]
  }
  
  if(tolower(method)=="mad"){
    get_topMAD_features(df_,N_genes)
  }
}
...
get_nonCorr_genes=function(obj_in,
  ...
  if(!is.null(featureRank)){
    #
    #cat(inlist)
    cat("Length of inlist:", length(inlist), "\n")
    cat("Length of featureRank:", length(featureRank), "\n")
...
# get_nonCorr_genes = function(obj_in, feature_panel = NULL, featureRank = NULL, highCorrCutoff = 0.75, iteration_times = NULL, assay_name_in = "vst") {
#   # Validate feature_panel input
#   if (!is.null(feature_panel) && !feature_panel %in% names(obj_in)) {
#     stop("Error: feature_panel not found in obj_in.")
#   }
#   
#   # Get input matrix
#   if (is.null(feature_panel)) {
...
#     indata = assays(obj_in$SE)[[assay_name_in]]
#     inlist = obj_in[[feature_panel]]
#     cat("Feature panel:", feature_panel, "\n")
#     cat("Initial inlist length:", length(inlist), "\n")
#     
#     # Limit inlist length to match featureRank
#     inlist = inlist[1:min(length(featureRank), length(inlist))]
#     cat("Adjusted inlist length:", length(inlist), "\n")
#     
#     if (length(inlist) == 0) {
#       stop("Error: 'inlist' is empty. Check if 'obj_in[[feature_panel]]' contains valid data.")
#     }
#     indata = indata[inlist, ]
#   }
#   
...
#     modelDF = t(indata) %>% as.data.frame()
#     cat("Calculating correlation...\n")
#     corrMatrix = cor(modelDF, use = "complete.obs")
#     highCorrIdx = caret::findCorrelation(corrMatrix, cutoff = highCorrCutoff)
#     highCorrGenes = colnames(modelDF[, highCorrIdx])
#     
#     # Output result
...
#     cat("Length of featureRank:", length(featureRank), "\n")
#     
#     if (length(featureRank) != length(inlist)) {
#       stop("Length of features and feature ranks do not match")
#     }
#     
#     df_feature = data.frame(feature = inlist, rank = featureRank, stringsAsFactors = FALSE)
#     
#     set.seed(10)
#     modelDF = t(indata) %>% as.data.frame()
#     
#     # Remove columns with zero standard deviation
#     modelDF = modelDF[, apply(modelDF, 2, sd) != 0]
#     
#     corrMatrix = cor(modelDF, use = "complete.obs") %>% reshape2::melt() %>% filter(!Var1 == Var2)
#     
#     if (max(corrMatrix$value, na.rm = TRUE) >= highCorrCutoff) {
#       message("Run correlation iteration")
#       
#       data_loop = indata
#       corrMatrix_loop = corrMatrix
...
#       in_list_ = inlist
#       
#       while (max(corrMatrix_loop$value, na.rm = TRUE) >= highCorrCutoff) {
#         cat(paste0("Iterations: ", i, "\n"))
#         
#         corrMatrix_loop1 = corrMatrix_loop %>% filter(!Var1 == Var2) %>%
#           filter(value >= highCorrCutoff) %>%
#           left_join(df_feature %>% transmute(Var1 = feature, rank1 = rank)) %>%
#           left_join(df_feature %>% transmute(Var2 = feature, rank2 = rank)) %>%
#           dplyr::mutate(keep_id = ifelse(rank1 < rank2, Var1, ifelse(rank1 > rank2, Var2, NA)))
#         
#         corrMatrix_loop1$keep_id[is.na(corrMatrix_loop1$keep_id)] = apply(corrMatrix_loop1[is.na(corrMatrix_loop1$keep_id), ], 1, function(x) { sort(c(unlist(x[1]), unlist(x[2])))[1] })
#         
#         remove_ids_ = unique(c(corrMatrix_loop1$Var1, corrMatrix_loop1$Var2))
#         remove_ids_ = remove_ids_[!remove_ids_ %in% corrMatrix_loop1$keep_id]
#         cat(paste0("\nRemoved ids: ", paste0(remove_ids_, collapse = ","), "\n"))
...
#         data_loop = data_loop[in_list_, ]
#         
#         corrMatrix_loop = as.data.frame(t(data_loop)) %>% cor(use = "complete.obs") %>% reshape2::melt() %>% filter(!Var1 == Var2)
#         i = i + 1
#         cat(paste0("Number of removed features: ", length(remove_ids), "\n"))
#         if (!is.null(iteration_times)) {
#           if ((i - 1) >= iteration_times) {
#             break
#           }
#         }
#       }
...
run_PCA=function(obj_in,dims=100,variable_n=800,feature_panel="TopMAD5000",out_label="pca",assay_name_in="vst"){
  ...
  variable_genes_in=obj_in[[feature_panel]][1:variable_n]
